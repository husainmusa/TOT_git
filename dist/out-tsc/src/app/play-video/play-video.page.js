...
var PlayVideoPage = /** @class */ (function () {
    function PlayVideoPage(router, route, zone, alertCtrl, loadingCtrl, elementRef, restProvider, audioProvider, keyboard) {
        var _this = this;
        this.router = router;
        this.route = route;
        this.zone = zone;
        ...
        this.visiblePlayerAndroid = false;
        this.visiblePlayerDeezer = false;
        ...
        this.time = "00:00";
        var v = this.restProvider.returnApp();
        console.log('v', v);
        if (v == 'Deezer') {
            this.visiblePlayerDeezer = true;
        ...
        this.route.queryParams.subscribe(function (params) {
            ...
            if (_this.router.getCurrentNavigation().extras.state) {
                ...
                _this.category = _this.router.getCurrentNavigation().extras.state.category;
                _this.equipImg = _this.router.getCurrentNavigation().extras.state.equipImg;
                _this.total_Set = _this.router.getCurrentNavigation().extras.state.sets;
                console.log('cat', _this.category);
                if (_this.category == "amrap") {
                    _this.disable_Back = true;
                    _this.overlay1 = true;
                    _this.amrap = true;
                }
                else if (_this.category == "emom") {
                    _this.overlay1 = true;
                    _this.disable_Back = true;
                    _this.equipSelect = true;
                }
                else {
                    _this.overlay = true;
                }
                console.log(_this.id, _this.type, _this.category);
            }
        });
    }
    ...
    PlayVideoPage.prototype.doSomethingWithCurrentValue = function (event) {
    };
    PlayVideoPage.prototype.closeKeyboard = function (event) {
        this.keyboard.hide();
        ...
        this.restProvider.api_method('get', 'get_workout_vids_detail/' + this.id, null, null).subscribe(function (data) {
            ...
            if (data['status'] == 'success') {
                ...
                if (data['data'].length > 0) {
                    ...
                    _this.all_video_detail = data;
                    _this.index = 0;
                    _this.thumbnail = data['horz_img'] ? data['horz_img'] : '../assets/images/vid_bg.jpg';
                    _this.setVideo(); //
                    ...
                    var t_set = _this.count_vid_list_length * _this.total_Set;
                    for (var i = 0; i < t_set; i++) {
                        _this.total_footer.push(i);
                    }
    ...
    PlayVideoPage.prototype.setVideo = function () {
        try {
            ...
            for (var _i = 0; _i < this.total_footer.length; _i++) {
                //console.log(this.index,'footer-indicator-',_i);
                if (document.getElementById('footer-indicator-' + _i)) {
                    document.getElementById('footer-indicator-' + _i).style.background = _i > this.count_footer ? 'var(--ion-color-light)' : 'var(--ion-color-primary)';
                }
            }
            this.count_footer++;
            if (this.video_first) {
    ...
    PlayVideoPage.prototype.startCountdown = function () {
        ...
        var interval = setInterval(function () {
            ...
            if (_this.counter < 1) {
                if (_this.category == "amrap" || _this.category == "emom") {
                    _this.setClock();
                }
                else {
                    _this.resetClock();
    ...
    PlayVideoPage.prototype.endWaitingTime = function () {
